...
contract DeployArcNameServiceV2 {
    ...
    struct Deployment {
        ...
        ArcBaseRegistrarV2 baseRegistrar;
    ...
    function _loadConfig() private view returns (DeploymentConfig memory config) {
        ...
        config.metadataBaseURI = _VM.envString("METADATA_BASE_URI");
    ...
    function _deployCore(Deployment memory deployed, DeploymentConfig memory config) private {
        ...
        deployed.baseRegistrar = new ArcBaseRegistrarV2(
            deployed.registry, _BASE_NODE, config.deployer, config.metadataBaseURI
        );
    ...
    function _assertSettlementAsset() private view {
        ...
        try IERC20MetadataV2(_ARC_USDC).decimals() returns (uint8 decimals) {
        ...
        view
    {
        ...
        if (
            deployed.registry.owner(bytes32(0)) != config.owner
                ...
                || !deployed.baseRegistrar.supportsInterface(0x01ffc9a7)
                || !deployed.baseRegistrar.supportsInterface(0x80ac58cd)
                || !deployed.baseRegistrar.supportsInterface(0x5b5e139f)
                || keccak256(bytes(deployed.baseRegistrar.metadataBaseURI()))
                    != keccak256(bytes(config.metadataBaseURI))
